    ...
    try {
      ...
      if (cfg) {
        setForm((prev) => ({
          ...
          apiKey: '', // no se devuelve por seguridad
        }));
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      await http.post(`/api/admin/ai-config`, {
        ...
        apiKey: form.apiKey, // requerido por el backend
        modelName: form.modelName,
        maxTokens: form.maxTokens,
        temperature: form.temperature,
      });
      setMessage('Configuración guardada correctamente.');
      // por seguridad, limpiamos el campo apiKey
      setForm((f) => ({ ...f, apiKey: '' }));
    } catch (e) {
      const err = e as AxiosError<any>;
...
function Analytics() {
  ...
  const [aiQ, setAiQ] = useState('Resúmeme las ventas por día y producto más vendido.');
  ...
  async function askAI() {
    try {
      const res = await http.post('/api/chat/message', { message: aiQ, sessionId: 'analytics' });
      setAiA(res.data?.data?.response || '');
    } catch {
      setAiA('No se pudo obtener respuesta.');
    }
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
      ...
      <div className="rounded-xl border border-border bg-surface p-5 shadow-card">
        <div className="text-sm text-mutedText mb-2">Asistente de Analíticas (IA)</div>
        <textarea className="w-full h-24 rounded-md border border-border px-3 py-2 mb-2" value={aiQ} onChange={(e) => setAiQ(e.target.value)} />
        <button className="h-9 px-4 rounded-md bg-black text-white hover:bg-black/90" onClick={askAI}>Preguntar</button>
        {aiA && <div className="mt-3 text-sm whitespace-pre-wrap">{aiA}</div>}
      </div>
    </div>
  );
}
