  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {[{ t: 'Productos', kpi: '—' }, { t: 'Usuarios', kpi: '—' }, { t: 'Chat IA', kpi: 'Configuración' }].map((c) => (
          <div key={c.t} className="rounded-xl border border-border bg-surface p-5 shadow-card">
            <div className="text-sm text-mutedText">{c.t}</div>
            <div className="mt-2 text-2xl font-bold text-text">{c.kpi}</div>
            <div className="mt-1 text-xs text-mutedText">Resumen general</div>
          </div>
        ))}
      </div>
