...
export default function App() {
  ...
  // Cerrar sesión de chat al cerrar o recargar la página (eliminar historial en servidor)
  useEffect(() => {
    function handleBeforeUnload() {
      try {
        // Evitar preflight: usar GET con query y sin cuerpo
        const url = `${API_BASE}/api/chat/close?sessionId=${encodeURIComponent(sessionId)}`;
        if (navigator.sendBeacon) {
          // sendBeacon requiere body, pero enviamos un blob vacío y trasladamos sessionId a query
      ...
      <div className="flex-1 overflow-y-auto">
        {messages.map((m, idx) => (
          <div key={idx} className={`flex items-end gap-3 p-4 ${m.role === 'user' ? 'justify-end' : ''}`}>
            {/* Avatares eliminados por requerimiento */}
